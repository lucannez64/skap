...
static SQL_INSERT_USER: &str =
    "INSERT INTO users (id, email, ky_public_key, di_public_key) VALUES ($1, $2, $3, $4)";
static SQL_SELECT_USER: &str =
    "SELECT email, ky_public_key, di_public_key FROM users WHERE id = $1";
static SQL_DELETE_USER: &str = "DELETE FROM users WHERE id = $1";
static SQL_SELECT_USER_BY_EMAIL: &str =
    "SELECT id, email, ky_public_key, di_public_key FROM users WHERE email = $1";
...
static SQL_INSERT_SHARED_PASS: &str =
    "INSERT INTO shared_passes (owner_id, pass_id, recipient_id, data) VALUES ($1, $2, $3, $4)";
static SQL_SELECT_SHARED_PASS: &str =
    "SELECT data FROM shared_passes WHERE owner_id = $1 AND pass_id = $2 AND recipient_id = $3";
static SQL_DELETE_SHARED_PASS: &str =
    "DELETE FROM shared_passes WHERE owner_id = $1 AND pass_id = $2 AND recipient_id = $3";
static SQL_SELECT_ALL_SHARED_PASSES: &str =
    "SELECT data, pass_id, owner_id FROM shared_passes WHERE recipient_id = $1";
static SQL_SELECT_SHARED_BY_USER: &str =
    "SELECT pass_id, recipient_id FROM shared_passes WHERE owner_id = $1";
static SQL_SELECT_SHARED_BY_USER_AND_PASS: &str =
    "SELECT recipient_id FROM shared_passes WHERE owner_id = $1 AND pass_id = $2";
static SQL_UPDATE_SHARED_PASS: &str =
    "UPDATE shared_passes SET data = $4 WHERE owner_id = $1 AND pass_id = $2 AND recipient_id = $3";
...
impl UsersPostgres {
    pub async fn new(
        url: &str,
        file: &str,
    ) -> Result<UsersPostgres, Box<dyn std::error::Error + Send + Sync>> {
        Ok(UsersPostgres {
...
impl UsersT for UsersPostgres {
    async fn add_user(&mut self, id: uuid::Uuid, user: crate::protocol::CK) -> ResultP<()> {
        ...
        let stmt = handle_db_error!(
            database.prepare_cached(SQL_INSERT_USER).await,
            "Error preparing add_user statement"
        )?;
        handle_db_error!(
            database
                .execute(&stmt, &[&id, &user.email, &user.ky_p, &user.di_p])
                .await,
            "Error adding user"
        )?;
        Ok(())
    ...
    async fn remove_user(&mut self, id: uuid::Uuid) -> ResultP<()> {
        let database = self.database.get().await?;
        handle_db_error!(
            database.execute(SQL_DELETE_USER, &[&id]).await,
            "Error removing user"
        )?;
        Ok(())
    }
    ...
    async fn get_user_from_email(&self, email: String) -> ResultP<crate::protocol::CK> {
        let database = self.database.get().await?;
        let stmt = handle_db_error!(
            database.prepare_cached(SQL_SELECT_USER_BY_EMAIL).await,
            "Error preparing get_user_from_email statement"
        )?;

        // Utiliser query_opt au lieu de query_one pour gérer le cas où l'email n'existe pas
        let row_opt = handle_db_error!(
            database.query_opt(&stmt, &[&email]).await,
    ...
    async fn get_uuids_from_emails(&self, emails: Vec<String>) -> ResultP<Vec<Uuid>> {
        let database = self.database.get().await?;
        let stmt = handle_db_error!(
            database.prepare_cached(SQL_SELECT_UUIDS_FROM_EMAILS).await,
            "Error preparing get_uuids_from_emails statement"
        )?;
        let rows = handle_db_error!(
            database.query(&stmt, &[&emails]).await,
            "Error getting uuids from emails"
        )?;
    ...
    async fn get_emails_from_uuids(&self, uuids: Vec<Uuid>) -> ResultP<Vec<String>> {
        let database = self.database.get().await?;
        let stmt = handle_db_error!(
            database.prepare_cached(SQL_SELECT_EMAILS_FROM_UUIDS).await,
            "Error preparing get_emails_from_uuids statement"
        )?;
        let rows = handle_db_error!(
            database.query(&stmt, &[&uuids]).await,
            "Error getting emails from uuids"
        )?;
        Ok(rows.iter().map(|r| r.get(0)).collect())
    }
    ...
    async fn get_public_key(
        &self,
        id: Uuid,
    ) -> ResultP<[u8; crate::protocol::KYBER_PUBLICKEYBYTES]> {
        let database = self.database.get().await?;
        let stmt = handle_db_error!(
            database.prepare_cached(SQL_SELECT_PUBLIC_KEY).await,
            "Error preparing get_public_key statement"
        )?;
...
impl PassesPostgres {
    pub async fn new(
        url: &str,
        file: &str,
    ) -> Result<PassesPostgres, Box<dyn std::error::Error + Send + Sync>> {
        Ok(PassesPostgres {
            database: Database::new(url, file).await?,
        })
    }
...
impl PassesT for PassesPostgres {
    async fn add_pass(
        ...
        let database = self.database.get().await?;
        let stmt = handle_db_error!(
            database.prepare_cached(SQL_INSERT_PASS).await,
            "Error preparing add_pass statement"
        )?;
        handle_db_error!(
            database.execute(&stmt, &[&id, &pass_id, &pass]).await,
            "Error adding pass"
        )?;
        Ok(())
    }
    ...
    async fn get_pass(&self, id: uuid::Uuid, pass_id: uuid::Uuid) -> ResultP<Vec<u8>> {
        let database = self.database.get().await?;
        let stmt = handle_db_error!(
            database.prepare_cached(SQL_SELECT_PASS).await,
            "Error preparing get_pass statement"
        )?;

        // Utiliser query_opt au lieu de query_one pour gérer le cas où le mot de passe n'existe pas
        let row_opt = handle_db_error!(
            database.query_opt(&stmt, &[&id, &pass_id]).await,
            "Error getting pass"
        )?;
    ...
    async fn remove_pass(&mut self, id: uuid::Uuid, pass_id: uuid::Uuid) -> ResultP<()> {
        let database = self.database.get().await?;
        handle_db_error!(
            database.execute(SQL_DELETE_PASS, &[&id, &pass_id]).await,
            "Error removing pass"
        )?;
        Ok(())
    }
    ...
    async fn update_pass(
        ...
        handle_db_error!(
            database
                .execute(SQL_UPDATE_PASS, &[&id, &pass_id, &pass])
                .await,
            "Error updating pass"
        )?;
        Ok(())
    ...
    async fn get_all_pass(&self, id: uuid::Uuid) -> ResultP<Vec<(Vec<u8>, uuid::Uuid)>> {
        let database = self.database.get().await?;
        let stmt = handle_db_error!(
            database.prepare_cached(SQL_SELECT_ALL_PASSES).await,
            "Error preparing get_all_pass statement"
        )?;
        let rows = handle_db_error!(
            database.query(&stmt, &[&id]).await,
            "Error getting all passes"
        )?;
...
impl SharedPassesPostgres {
    pub async fn new(
        url: &str,
        file: &str,
    ) -> Result<SharedPassesPostgres, Box<dyn std::error::Error + Send + Sync>> {
...
impl SharedPassesT for SharedPassesPostgres {
        ...
        if let Ok(_) = self.get_shared_pass(recipient, owner, pass_id).await {
            ...
            handle_db_error!(
                database
                    ...
                    .await,
                "Error updating existing shared pass"
            )?;
        } else {
            let stmt = handle_db_error!(
    ...
    async fn get_shared_pass(
        ...
        let row_opt = handle_db_error!(
            database
                .query_opt(&stmt, &[&owner, &pass_id, &recipient])
                .await,
    ...
    async fn get_shared_by_user_and_pass(&self, owner: Uuid, pass_id: Uuid) -> ResultP<Vec<Uuid>> {
        ...
        let stmt = handle_db_error!(
            database
                .prepare_cached(SQL_SELECT_SHARED_BY_USER_AND_PASS)
                .await,
